...
public class IndexTableQueryUser extends MongoDBWorkload {
	...
	public boolean doTransaction(final DB db, final Object threadState) {
		...
		String email = String.valueOf(Utils.hash(keybase.getAndIncrement())); 
		long start = System.currentTimeMillis();		
		
		try {
			UUID userId = ((FindIterable<Document>) Submitter.submit(tries, maxBackoff,
					() -> mongoDatabase.getCollection("email").find(eq("_id", email)))).first().get("user_id", UUID.class);
			((FindIterable<Document>) Submitter.submit(tries, maxBackoff,
					() -> mongoDatabase.getCollection("user").find(eq("_id", userId)))).first();
